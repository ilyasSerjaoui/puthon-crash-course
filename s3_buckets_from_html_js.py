    ...
    for subdomain in subdomains.readlines():
        ...
        if urls:
            for url in urls:
                print("checking - "+url)
                try:
                    html = request.get(url=url,timeout=10,verify=False).content
                    try:
                        html = unquote(str(html))
                    except Exception as e:
                        print(e)
                    regjs = r"(?<=src=['\"])[a-zA-Z0-9_\.\-\:\/]+\.js"
                    regs3 = r"[\w\-\.]+\.s3\.?\.amazonaws\.com|(?<!\.)s3\.?(?:[\w\-\.]+).\.amazonaws\.com\\?\/[\w\-\.]+"
                    js=re.findall(regjs,html)
                    s3=re.findall(reqs3,html)
                    buckets=buckets+s3
                    if len(js)>0:
                        for i in js:
                            if i.startswith('//'):
                                ...
                                jsurl=url+'/'+i
                            try:
                                jsfile = request.get(jsurl,timeout=10).content
